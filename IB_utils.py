...
## 상장기업 kind
def go_kind(driver, start_dt, end_dt, code = None, FIRST = True):
    time.sleep(0.25)
    ...
    ## 03. 기간 설정
    # driver.find_element(By.CLASS_NAME, 'ord-07').click() # 전체
    start_element = driver.find_elements(By.ID, 'fromDate')[0]
    start_element.clear()
    time.sleep(0.1)
    start_element.send_keys(start_dt)
    time.sleep(0.1)
    start_element.send_keys(Keys.ESCAPE)
    time.sleep(0.1)
        
    end_element = driver.find_elements(By.ID, 'toDate')[0]
    end_element.clear()
    time.sleep(0.1)
    end_element.send_keys(end_dt)
    time.sleep(0.1)
    end_element.send_keys(Keys.ESCAPE)
...
def kind_main_ib(driver, dart_df, start_dt, end_dt):
    ## 01.KIND 접속
    ...
    # kind 접속
    df = go_kind(driver, start_dt, end_dt)

    rows = driver.find_elements(By.XPATH, '/html/body/section[2]/section/article/section[1]/table/tbody/tr')

    for link, code in zip(rows, df['회사명']):
        link.click()
        time.sleep(1)
    ...
    first_df = pd.merge(dart_df.loc[:, select_cols], kind_df, left_on = 'corp_name',  right_on ='stock_code', how = 'inner')
    ...
    del first_df['수요예측일정'], first_df['공모청약일정'], first_df['stock_code_y']
    first_df = first_df.rename(columns = {'stock_code_x':'stock_code'})
...
def get_38_outer():
    ...
    response = requests.get(url)
    soup = BeautifulSoup(response.text, 'html.parser')
    tb_src = soup.select('table[summary="수요예측일정"]')

    temp_df = pd.read_html(str(tb_src))[0]
    temp_df = temp_df.loc[~temp_df['종목명'].isna()]
    del temp_df['Unnamed: 6'], temp_df['Unnamed: 7']
